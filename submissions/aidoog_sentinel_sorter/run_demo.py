...
BASE_TASKS = (
    ...
    SortTask(
        name="silver_micro_screw",
        freejoint="silver_micro_screw_freejoint",
        body="silver_micro_screw",
        start=(-0.015, -0.075, 0.052),
        bin_center=(0.21, 0.10, 0.058),
        label="silver_micro_screw_to_precision_tray",
        object_type="micro_screw",
        carry_height=0.054,
    ),
)
...
def plan_at(time_s: float, duration_s: float, tasks: tuple[SortTask, ...]) -> dict:
    ...
    policy_confidence = 0.89 + 0.105 * smoothstep(0.18, 0.34, local_t)
    vision_confidence = 0.975 + 0.024 * smoothstep(0.0, 0.10, local_t)
    ...
    return {
        ...
        "vision_confidence": min(0.999, vision_confidence),
...
def advanced_evidence_metrics(logs: list[dict]) -> dict:
    ...
    return {
        ...
        "manipulation_modes": ["six-target triage", "five-finger grasp", "micro-screw precision pickup", "ellipsoid stable grasp", "slip recovery", "216-degree cap rotation", "9x load hold", "decoy rejection", "occlusion-aware transport"],
...
def caption_for_plan(plan: dict, suite: dict | None = None) -> str:
    if plan["task"].name == "red_cube":
        ...
        task_label = "PURPLE"
    elif plan["task"].name == "silver_micro_screw":
        task_label = "SILVER"
    else:
        task_label = "AMBER"
    display_phase = {
...
def run_demo(
    *,
    ...
    summary = {
        ...
        "task_goal": "Autonomously triage six target object types through a cluttered randomized MuJoCo lab with nine distractors/decoys, partial occlusion, narrow-aisle transport, and recorded controls, vision confidence, five-finger tactile state, cap rotation, labels, poses, and success metrics.",
