...
- 15/15 dexterity skill checks passed in `data/evaluation_report.json`.
- 20/20 verification gates passed in `data/rollout_summary.json`.
- Four-object triage with five-finger grasp, 216-degree cap rotation, 0.36 mm slip recovery, and 9x load hold.
...
- The script writes `data/sensor_log.csv`, `data/rollout_summary.json`, `data/evaluation_report.json`, `data/behavior_policy.json`, and `data/randomized_layouts.json` for reproducibility, scoring evidence, and dataset review.

## Skill suite

| # | Check | Type |
|---|-------|------|
| 1 | red cube sort | object sorting |
| 2 | blue cylinder sort | object sorting |
| 3 | amber capsule sort | object sorting |
| 4 | green sphere sort | object sorting |
| 5 | five-finger tactile grasp | dexterity |
| 6 | 216-degree cap rotation | dexterity |
| 7 | slip recovery | stability |
| 8 | 9x load hold | force control |
| 9 | six-distractor clearance | navigation |
| 10 | randomized layout seed | generalization |
| 11 | vision confidence above 98% | perception |
| 12 | policy confidence above 93% | control |
| 13 | minimum-jerk transport | trajectory |
| 14 | synchronized sensor export | engineering |
| 15 | composite 20-gate suite | integration |
...
- 15/15 skill-suite report with control, perception, dexterity, stability, navigation, trajectory, and integration checks.
...
submissions/aidoog_sentinel_sorter/data/evaluation_report.json
...
Recommended review path: open the video first, then inspect `data/evaluation_report.json`, `data/rollout_summary.json`, and `data/sensor_log.csv`.
