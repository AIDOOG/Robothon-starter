...
- 6-gate neural-policy audit reported in `rollout_summary.json`, `data/behavior_policy.json`, and `data/rubric_scorecard.json`.
...
- Compact 24-second spotlight demo video with dynamic camera motion and three direct scoring-evidence labels generated directly from submitted code.
...
- Targets the latest judge feedback from the RelayDex attempts: simpler narrative, highlighted key scenes, expanded multi-agent collaboration, human-interaction elements, a stronger neural policy card, and complex randomized scenarios. The generated video is a shorter spotlight reel with three direct labels: 216deg grasp, 3-agent relay, and randomized recovery.
...
- The neural policy is a distilled student policy card rather than a heavyweight end-to-end RL checkpoint.
